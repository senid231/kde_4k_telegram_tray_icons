...
dot_size=50 # measure in % (smallest than the size)
font_size=145 # measure in % (smallest than the dot size)
...
mkdir ticons
cp ${source} ticons/icon_22_0.png
cp ${source} ticons/iconmute_22_0.png
...
for i in $(seq 1 ${messages}); do
  ...
  echo "${dot//##/#FF0000}" | convert -background transparent -gravity Center -font ${font} -pointsize ${psize}% -fill white -draw "text 0,0 ${counter//##/$i}" svg:- png:- | composite -density ${dpi} -gravity SouthEast png:- ${source} ticons/icon_22_${i}.png
  echo "${dot//##/#555555}" | convert -background transparent -gravity Center -font ${font} -pointsize ${psize}% -fill white -draw "text 0,0 ${counter//##/$i}" svg:- png:- | composite -density ${dpi} -gravity SouthEast png:- ${source} ticons/iconmute_22_${i}.png
done
